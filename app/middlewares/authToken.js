...
module.exports = function (req, res, next) { // usaremos next para dar acceso
    if (req.path != '/auth/login') { // cualquiera puede logearse
        if (req.headers.authorization) { // con esta función compruebo el token que se encuentra en los headers
            ...
            secretToken.verify(token, CONFIG.SECRET_KEY_TOKEN, function (error, decoded) {
                ...
                if (req.method != 'GET') { //Ambas rutas tienen el método GET y por ahí puedo tomarlas
                    if (decoded.role === 'admin') next();
                    else res.status(403).send({ message: 'Acceso denegado, no tienes permisos suficientes' })
                } else {
                    next();
                }
            });
